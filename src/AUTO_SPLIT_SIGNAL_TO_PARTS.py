...
output_folder_mic1_template = './LONG_TDM_{}/mic1/'
output_folder_mic2_template = './LONG_TDM_{}/mic2/'
output_folder_mic3_template = './LONG_TDM_{}/mic3/'
...
for i, segment in enumerate(segments1):
    ...
    segment[['Time', 'Amplitude']].to_csv(output_folder + f'wear1-1_timeDM.csv', index=False)

for i, segment in enumerate(segments2):
    output_folder = os.path.join(output_folder_mic2_template.format(i))
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    segment[['Time', 'Amplitude']].to_csv(output_folder + f'wear1-2_timeDM.csv', index=False)

for i, segment in enumerate(segments3):
    output_folder = os.path.join(output_folder_mic3_template.format(i))
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    segment[['Time', 'Amplitude']].to_csv(output_folder + f'wear1-3_timeD.csv', index=False)
