...
for j in range(5, 6):  # Loop for j from 1 to 4
	print(f'處理第 {j} 組數據...')

	# 設置輸入文件夾
	INPUT_FOLDER = f'./RMS_TDM_{j}/'
	...
	for i in range(1, 31):  # 根據需要更改範圍
		for k in range(1, SECEOND_LENGTH+1):
			print(f'處理第 {i} 組數據...')

			# 讀取三個麥克風的數據
			mic_files = {
				'mic1': f'./{INPUT_FOLDER}/mic1/wear{i}-1_timeDM_{k}s.csv',
				'mic2': f'./{INPUT_FOLDER}/mic2/wear{i}-2_timeDM_{k}s.csv',
				'mic3': f'./{INPUT_FOLDER}/mic3/wear{i}-3_timeDM_{k}s.csv'
			}
			if not os.path.exists(mic_files["mic1"]):
				print(f"文件 {mic_files["mic1"]} 不存在，跳過。")
				continue
			if not os.path.exists(mic_files["mic2"]):
				print(f"文件 {mic_files["mic2"]} 不存在，跳過。")
				continue
			if not os.path.exists(mic_files["mic3"]):
				print(f"文件 {mic_files["mic3"]} 不存在，跳過。")
				continue
			...
			output_file_path = os.path.join(OUTPUT_FOLDER, f'micAll_wear{i}_filtered_{k}s.csv')
