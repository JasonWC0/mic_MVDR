...
for i in range(5, 6):
    DATA_FOLDER = f"./RMS_TDM_{i}/"
    OUTPUT_BASE_FOLDER = f"./TDM_ADJESTED_{i}/"
    HIGH_PASS_CUTOFF = 20
    LOW_PASS_CUTOFF = 20000
    FS = 51200
    FIRST_EXCEL = "wear1-{}_timeDM_1s.csv"
    ...
    first_data = pd.read_csv(
        os.path.join(DATA_FOLDER, "mic1", FIRST_EXCEL.format(1)),
        header=None,
    ...
    first_data = pd.read_csv(
        os.path.join(DATA_FOLDER, "mic2", FIRST_EXCEL.format(2)),
        header=None,
        names=["Time", "Amplitude"],
        skiprows=1,
    )
    rms2 = np.sqrt(np.mean(first_data["Amplitude"] ** 2))

    first_data = pd.read_csv(
        os.path.join(DATA_FOLDER, "mic3", FIRST_EXCEL.format(3)),
        header=None,
        names=["Time", "Amplitude"],
        skiprows=1,
    )
    rms3 = np.sqrt(np.mean(first_data["Amplitude"] ** 2))
    # ------------------------------------------
    ...
    for f1, f2, f3 in zip(mic1_files, mic2_files, mic3_files):
        ...
        data1.to_csv(
            os.path.join(OUTPUT_BASE_FOLDER, "mic1", os.path.basename(f1)),
            index=False,
            header=True,
        )
        data2.to_csv(
            os.path.join(OUTPUT_BASE_FOLDER, "mic2", os.path.basename(f2)),
            index=False,
            header=True,
        )
        data3.to_csv(
            os.path.join(OUTPUT_BASE_FOLDER, "mic3", os.path.basename(f3)),
            index=False,
            header=True,
        )
        counter += 1
        ...
        print(
            f"Processed and saved: {os.path.basename(f1)}, {os.path.basename(f2)}, {os.path.basename(f3)}"
        )
