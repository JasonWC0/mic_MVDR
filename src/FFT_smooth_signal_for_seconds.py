...
OUTPUT_FILE_DIR='TDM5_FFT_SMOOTH/mic1'
INPUT_DIR = f'././{TARGET_FILE_NAME}'
PROCESS_CSV_FILE_NAME = 'wear{}-1_timeDM_{}s.csv'

output_dir = f'././{OUTPUT_FILE_DIR}'
SECEOND_LENGTH = 17
...
# 處理多個文件
for i in range(1, 21):
    for j in range(1,SECEOND_LENGTH+1):

        file_path = os.path.join(INPUT_DIR, PROCESS_CSV_FILE_NAME.format(i,j))
        ...
        output_csv_path = os.path.join(output_dir, PROCESS_CSV_FILE_NAME.format(i,j))
